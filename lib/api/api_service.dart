...
final _logger = Logger('app.anlage.game.api.api_service');

class LoginState {
  LoginState(Uri baseUri, this.userInfo) : avatarUrl = baseUri.resolve(userInfo.avatarUrl).toString();
...
class ApiNetworkError extends Error {
  ...
  String toString() {
    ...
    if (cause.response?.statusCode == HttpStatus.badRequest) {
      data = cause?.response?.data as dynamic;
    }
...
class NativeService {
  NativeService._(MethodChannel channel) : _channel = channel;
  ...
  Future<bool> isFirebaseTestLab() {
    ...
    return _channel.invokeMethod<String>('getString', {'name': 'firebase.test.lab'}).then((value) => value == 'true');
  }
}
...
class ApiService {
  ...
  Uri _baseUri;
  ValueObservable<LoginState> get loginState => _loginState.stream;
  ...
  Future<void> _updateUserInfo({int retryCount = 0}) async {
    ...
    if (Platform.isIOS) {
      ...
      deviceInfo =
          '$testLab${androidInfo.model},${androidInfo.brand},${androidInfo.device},${androidInfo.board},${androidInfo.manufacturer},${androidInfo.product},${androidInfo.version.baseOS},${androidInfo.version.release}';
    }
    ...
    final appVersion =
        '${packageInfo.version} (${packageInfo.buildNumber}) ${packageInfo.packageName} ${packageInfo.appName}';
    _logger.finer('sending userinfo.');

    try {
      final userInfo = await _apiCaller.post(
          UserInfoLocation(), UserInfoRequest(appVersion, deviceInfo, await _cloudMessaging.getToken()));
      ...
      await AnalyticsUtils.instance.analytics.setUserProperty(
          name: 'user_type',
          value: testlabUserProperty == null ? userTypeString : testlabUserProperty + ':' + userTypeString);
      ...
      if (retryCount < 10) {
        ...
        _logger.warning(
            'Error while updating user info. retrying in $duration seconds. Retries: $retryCount', error, stackTrace);
        // retry later in the background.. do not wait for the response.
        unawaited(Future<dynamic>.delayed(Duration(seconds: duration)).then((dynamic val) {
...
//    response.read
  ...
  Future<GameSimpleSetVerifyResponse> verifySimpleGameSet(
      String gameTurnId, List<GameSimpleSetGuessDto> guesses) async {
  ...
  Future<UserInfoResponse> uploadAvatarImage(File image) async {
    final formData = FormData.from(<String, dynamic>{'avatarImage': UploadFileInfo(image, basename(image.path))});
